...
@Component({
  ...
  selector: 'app-paypal',
  template: '<div #payPalScriptElem></div><div #payPalButtonContainerElem [id]="payPalButtonContainerId"></div>',
})
export class PaypalComponent implements OnChanges, AfterViewInit, OnDestroy {
  ...
  private setupScript(): void {
    ...
    window[this.paypalWindowName].Button.render({
      ...
      locale: this.translate.currentLang === 'es' ? 'es_AR' : 'en_US',
      ...
      onAuthorize: (data: IPayPalPaymentCompleteData, actions: any) => {
        ...
        if (this.config.integrationType === PayPalIntegrationType.ClientSideREST) {
          ...
          return actions.payment.execute().then((payment) => {
            ...
            this.config.onPaymentComplete(data, actions, payment);
          });
        }
      },
